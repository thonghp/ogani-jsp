...
function deleteState() {
	...
	$.ajax({
		...
		changeFormStateToNew();
		showToastMessage("Thành phố đã xoá thành công");
	}).fail(function() {
...
function updateState() {
	...
	$.ajax({
		...
		showToastMessage("Thành phố đã được cập nhật thành công");
		changeFormStateToNew();
	}).fail(function() {
		showToastMessage("ERROR: Could not connect to server or server encountered an error");
	});	
...
function addState() {
	...
	$.ajax({
		...
		showToastMessage("Thành phố đã được thêm thành công");
	}).fail(function() {
		showToastMessage("ERROR: Could not connect to server or server encountered an error");
	});
...
function loadStates4Country() {
	...
	$.get(url, function(responseJSON) {
		...
		changeFormStateToNew();
		showToastMessage("Thành phố đã tải lên thành công " + selectedCountry.text());
	}).fail(function() {
		showToastMessage("ERROR: Could not connect to server or server encountered an error");
	});	
}
